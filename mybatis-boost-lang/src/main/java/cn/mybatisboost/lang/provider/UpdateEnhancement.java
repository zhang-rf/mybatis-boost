...
public class UpdateEnhancement implements SqlProvider, ConfigurationAware {
        ...
        if (mappedStatement.getSqlCommandType() == SqlCommandType.UPDATE &&
            ...
            if (Objects.equals(literalColumns, "*") || literalColumns.toUpperCase().startsWith("NOT ")) {
                ...
                properties.removeAll(getConditionProperties
                        (boundSql.getParameterMappings(), split, entityType, mapUnderscoreToCamelCase));
    ...
    private List<String> getConditionProperties(List<ParameterMapping> parameterMappings,
                                                String[] split, Class<?> entityType, boolean mapUnderscoreToCamelCase) {
        ...
        if (split.length == 2) {
            if (!parameterMappings.isEmpty()) {
                ...
                conditionProperties = EntityUtils.getPropertiesFromColumns
                        (entityType, SqlUtils.findColumnsFromSQL(split[1]), mapUnderscoreToCamelCase);
            }
